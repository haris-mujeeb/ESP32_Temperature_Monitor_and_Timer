...
public:
  ...
  void clearEEPROM() {
      // Clear the EEPROM by writing zeros to the relevant addresses
    EEPROM.put(MAGIC_NUMBER_ADDR, (uint32_t)0);
    EEPROM.put(TEMP_THRESHOLD_ADDR, (int)0);
    EEPROM.put(TIME_THRESHOLD_ADDR, (uint32_t)0);
    EEPROM.put(TIME_ELAPSED_ADDR, (uint32_t)0);
